...
//backend server A UDP socket
int sockfd;

//server A address, mainserver UDP address
struct sockaddr_in serverA_addr, mainserver_UDP_addr;

//opens data1.txt and parses the file
void parseFile()
{
    std::ifstream data1_file;
    data1_file.open("./testcase1/data1.txt");
...
void setup_UDP_socket()
{
    ...
    //bind socket
    if (bind(sockfd, (struct sockaddr* )& serverA_addr, sizeof(serverA_addr)) < 0)
    {
        perror("bind failure");
    }
...
void send_UDP_countries()
{
    ...
    std::cout << "The server A has sent a country list to Main Server" << std::endl;
}
...
void process_user_query()
{
    ...
    std::cout << "The server A has received request for finding possible friends of User" << userID << " in " << country << std::endl;
    ...
    if (graphs[country].find(id) == graphs[country].end())
    {
        ...
        std::cout << "The server A has sent “User" << userID << " not found” to Main Server" << std::endl;
    }
    ...
    else
    {
        ...
        std::cout << "The server A has sent the result to Main Server" << std::endl;
...
int main()
{
    std::cout << "The server A is up and running using UDP on port 30481" << std::endl;
    parseFile();
